...
- Each file must reference the exact release version and source archive checksum (sha256) so that the package can be built reproducibly from the tagged release.
...
- [ ] **§5.5** Package distribution: `packaging/guix.scm`, `packaging/default.nix`, and `packaging/PKGBUILD` present, buildable, and carrying correct version + sha256 before any release tag is pushed
