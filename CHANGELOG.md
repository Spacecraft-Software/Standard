...
- **v1.50 (2026-08-18):** **§15.4 added — Authoring Model Attribution.** §15 has carried two attribution axes since the standard's early versions and stopped there: §15.2 records who maintains an artifact, §15.3 records whose work it stands on, and nothing anywhere recorded *what produced it*. That gap is invisible while a session is open and total once it closes — a year after a model drafts a PRD, an implementation plan, or a task list, or codes a project outright, the question that actually matters (which model, at what reasoning level, served by which provider, driven by which harness) has no answer in the repository. The one convention already in use, the `Co-Authored-By:` commit trailer, is the wrong shape for it: it marks every individual edit, which is activity rather than provenance; it does not survive a squash-merge or the history rewrites §6.3 permits; and it names a product without the reasoning level, the inference provider, or the harness. §15.4 requires the opposite — **one signature per artifact, written once**. Four triggers obligate it (a PRD, an implementation plan, a TODO or task list that drives implementation, or a project whose initial implementation was substantially produced by a model), and the section says plainly what does not: individual commits, edits, fixes, refactors and reviews, uncommitted scratch output, and a model that only reviewed or reformatted work it did not author. The record is a table — Model, Provider, Harness (where known), Date, Scope — carried as a document's final `Authoring Model` section, or, for a project, in a new **`AUTHORS.md` at the root**. `AUTHORS.md` is deliberately the provenance counterpart to §15.3's `CREDITS.md` rather than a section inside it: `CREDITS.md` records whose work the project stands on, `AUTHORS.md` records what produced the project's own text, and folding self-authorship into the inbound-credit file would muddy a convention already deployed across some twenty-five projects. It is likewise conditional — a hand-written project ships no `AUTHORS.md`. Four clauses close the section: the reasoning level belongs *in* the Model cell written as the vendor writes it (`glm-5.3-high`, `claude-opus-5 (effort: high)`), because the same weights at a different level produce materially different work; provider and harness are independent and both are recorded, since it is the combination that makes a result reproducible; a handoff **appends a row and never replaces one**, the table being a chronological record; and a signature is **not an authorship claim** — it MUST NOT appear in an `SPDX-FileCopyrightText` tag, a `# Maintainer:` line, or `--version` output, and never displaces the §15.2 attribution block. Nothing else moves: §15.2, §4.3, and the existing `Co-Authored-By:` trailer convention in the Construct repo are unaffected, and §15.4 sits beside the trailer rather than replacing it — the two answer different questions. The §15 chapter menu gains a fourth entry, §16 gains a matching checklist bullet marked N/A for hand-written artifacts, and the skill encoding lands as its own PR at this same version and date.
